...
$required = @("Name", "Username", "Password", "Email", "Group")
...
# $xmlout.WriteStartElement("Password");
# $xmlout.WriteAttributeString("passwordform", "encrypt");
# $xmlout.WriteString($password);
...
foreach ($line in $csv) {
  ...
  $xmlout.WriteElementString('Description', $line.Description)
  $xmlout.WriteStartElement('EmailList')
  $xmlout.WriteElementString('EmailID', $line.'Email')
  $xmlout.WriteEndElement()
  ...
  $xmlout.WriteElementString('SimultaneousLoginsGlobal', "Disable")
